...
package body Controls is
    ...
    Procedure Compute_Matrices_From_Inputs
       (Window            : in out Glfw.Windows.Window;
        Projection_Matrix : in out GL.Types.Singles.Matrix4;
        View_Matrix       : in out GL.Types.Singles.Matrix4)
    is
        use GL.Types;
    ...
    begin
        ...
        Horizontal_Angle := Horizontal_Angle + Mouse_Speed * Delta_Time * (Half_Window_Width - Single (X_Position));
        Vertical_Angle   := Vertical_Angle   + Mouse_Speed * Delta_Time * (Half_Window_Height - Single (Y_Position));
        ...
        if    Window'Access.Key_State (Keys.Up)    = Pressed then Position := Position + Direction * Delta_Time * Speed;
        elsif Window'Access.Key_State (Keys.Down)  = Pressed then Position := Position - Direction * Delta_Time * Speed;
        elsif Window'Access.Key_State (Keys.Right) = Pressed then Position := Position + Right * Delta_Time * Speed;
        elsif Window'Access.Key_State (Keys.Left)  = Pressed then Position := Position - Right * Delta_Time * Speed;
        end if;

        Maths.Init_Perspective_Transform (View_Angle, Single (Window_Width), Single (Window_Height), 0.1, 100.0, Projection_Matrix);
        Maths.Init_Lookat_Transform (Position, Direction, Up, View_Matrix);
